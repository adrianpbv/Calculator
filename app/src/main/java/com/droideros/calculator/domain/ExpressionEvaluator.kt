...
class ExpressionEvaluator(
    ...
    private fun evalFactor(expression: List<ExpressionPart>): ExpressionResult {
        return when(val part = expression.firstOrNull()) {
            ...
            // run will execute after finishing evaluating the expression
            ExpressionPart.Op(Operation.SUBTRACT) -> {
                evalFactor(expression.drop(1)).run {
                    ExpressionResult(
                        remainingExpression,
                        -value
                    ) // remainingExpression is from the evalFactor function (ExpressionResult.remainingExpression)
                }
            }
